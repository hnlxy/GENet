    ...
    def _prepare_base_model(self, backbone):
        ...
        if 'resnet' in backbone:
            if self.non_local:
                ...
                if self.net == 'M4':
                    assert self.cdiv >= 4
                    base_model_name = 'nets.GENet'
                    GENet = import_module(base_model_name)
                    print('=> base model: {}, with backbone: {}, loop: {}'.format(base_model_name, backbone, self.loop))
                    self.base_model = getattr(GENet, backbone)(pretrained=True if self.pretrain == 'imagenet' else False,
                    ...
                    GENet = import_module(base_model_name)
                    block = 'GE_%s'%self.net
                    self.base_model = getattr(GENet, backbone)(pretrained=True if self.pretrain == 'imagenet' else False, EF=block,
                            cdiv=self.cdiv, num_segments=self.num_segments, loop=self.loop)
